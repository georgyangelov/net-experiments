...
fn chunk_hash_file(path: &str, chunk_size: usize, read_size: usize) -> Vec<blake3::Hash> {
    ...
    let bufs = ReusableBuffers::new();
    let (send, recv) = std::sync::mpsc::sync_channel(buffer_size);

    let result = std::thread::scope(|s| {
        let bufs_ref = &bufs;
        s.spawn(move || chunker(path, read_size, send, bufs_ref));

        hasher(recv, chunk_size, &bufs)
    });

    println!("Total buffers used: {}", bufs.debug_len());

    result
}

fn chunker(path: &str, read_chunk_size: usize, send: SyncSender<Vec<u8>>, bufs: &ReusableBuffers) {
...
fn hasher(recv: Receiver<Vec<u8>>, hash_chunk_size: usize, bufs: &ReusableBuffers) -> Vec<blake3::Hash> {
...
struct ReusableBuffers {
    // TODO: Maybe we can use some concurrent VecDeque? Or a lock-free list?
    buffers: Mutex<Vec<Vec<u8>>>,
}

impl ReusableBuffers {
    fn new() -> ReusableBuffers {
